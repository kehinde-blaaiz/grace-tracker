...
export default function NotificationSettings({ profile, updateProfile }) {
  ...
  const handleToggle = async () => {
    ...
    if (!notificationsOn) {
      ...
      setRequesting(false)
      // Toggle on regardless — even without permission, show the UI
      // (reminders will just silently not fire if denied)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      <div style={{ background: '#fff', borderRadius: 16, border: '0.5px solid #e8e6e2', overflow: 'hidden' }}>
        ...
        {/* Main row */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderBottom: notificationsOn ? '0.5px solid #f0ede6' : 'none' }}>
          <div style={{ width: 32, height: 32, borderRadius: 10, background: notificationsOn ? '#EAF3DE' : '#f0f0ee', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <i className="ti ti-bell" style={{ fontSize: 16, color: notificationsOn ? '#1a3a0a' : '#aaa' }} aria-hidden="true" />
          </div>
          <div style={{ flex: 1 }}>
          ...
          {!isUnsupported && (
            <Toggle isOn={notificationsOn} onClick={handleToggle} size="lg" />
          )}
        </div>

        {/* Prayer time slots — only show when on */}
        {notificationsOn && (
          <>
            {[
            ...
            {isDenied && (
              <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                <i className="ti ti-alert-circle" style={{ fontSize: 13, color: '#A32D2D', flexShrink: 0 }} aria-hidden="true" />
            ...
            {!isDenied && (
              <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                <i className="ti ti-info-circle" style={{ fontSize: 13, color: '#888', flexShrink: 0 }} aria-hidden="true" />
