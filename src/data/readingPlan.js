...
function flatChapters(books, startBook) {
  const idx = books.findIndex(b => b.name === startBook)
  const selected = idx >= 0 ? books.slice(idx) : books
  const chapters = []
  selected.forEach(book => {
    for (let ch = 1; ch <= book.chapters; ch++) chapters.push({ book: book.name, chapter: ch })
  })
  return chapters
}

// testament: 'ot' | 'nt' | 'both-sequential' | 'both-parallel'
// For parallel: chaptersPerDay split evenly between OT and NT (min 2)
export function generatePlan(startBook = 'Mark', chaptersPerDay = 1, testament = 'nt', startOTBook = 'Genesis', startNTBook = 'Matthew') {
  const dayNames = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']
  const weeks = []

  if (testament === 'both-parallel') {
    // Split chapters between OT and NT each day
    const otPerDay = Math.ceil(chaptersPerDay / 2)
    const ntPerDay = Math.floor(chaptersPerDay / 2) || 1
    const otChapters = flatChapters(OT_BOOKS, startOTBook)
    const ntChapters = flatChapters(NT_BOOKS, startNTBook)

    let otIdx = 0, ntIdx = 0, weekNum = 1

    while ((otIdx < otChapters.length || ntIdx < ntChapters.length) && weekNum <= 500) {
      const days = []
      for (let d = 0; d < 5; d++) {
        const dayOT = otChapters.slice(otIdx, otIdx + otPerDay)
        const dayNT = ntChapters.slice(ntIdx, ntIdx + ntPerDay)
        if (!dayOT.length && !dayNT.length) break
        const allChapters = [...dayOT, ...dayNT]
        const label = allChapters.map(c => `${c.book} ${c.chapter}`).join(', ')
        days.push({ dayIndex: d, dayName: dayNames[d], chapters: allChapters, label, readSSLesson: d === 4 })
        otIdx += otPerDay
        ntIdx += ntPerDay
      }
    ...
    // Sequential — OT, NT, or both straight through
    let books
    if (testament === 'ot') books = OT_BOOKS
    else if (testament === 'nt') books = NT_BOOKS
    else books = ALL_BOOKS // both-sequential

    const allChapters = flatChapters(books, startBook)
    let idx = 0, weekNum = 1
    ...
    while (idx < allChapters.length && weekNum <= 500) {
      ...
      for (let d = 0; d < 5; d++) {
        ...
        days.push({ dayIndex: d, dayName: dayNames[d], chapters: dayChapters, label: dayChapters.map(c => `${c.book} ${c.chapter}`).join(', '), readSSLesson: d === 4 })
      }
      weeks.push({ weekNum, ssLesson: SS_LESSON_BASE + weekNum - 1, days })
      idx += readingsPerWeek
      weekNum++
